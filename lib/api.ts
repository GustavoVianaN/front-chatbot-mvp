...
export async function connectWhatsappCloud(input: { code: string; wabaId: string; phoneNumberId?: string; registrationPin?: string }): Promise<{ connected: boolean; coexistence?: boolean }> {
...
export async function disconnectWhatsappCloud(): Promise<{ connected: boolean }> {
  return apiRequest('/whatsapp-cloud/disconnect', {
    method: 'POST',
  });
}
